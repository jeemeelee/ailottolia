...
<section><h2>주간 프롬프트</h2><p class="muted">향후 AI 연동용 관리 설정입니다. 현재 무작위 번호 생성에는 적용되지 않습니다. 저장 즉시 관리 설정이 갱신되며 자동 주간 예약 기능은 없습니다.</p><form id="promptForm"><label for="prompt">프롬프트 내용 (최대 4,000자)</label><textarea id="prompt" maxlength="4000" required></textarea><p id="updated" class="muted"></p><button id="save">프롬프트 저장</button></form></section>
...
async function load(initial=false){
 ...
 if(initial){document.getElementById('prompt').value=data.prompt?.text||'';document.getElementById('updated').textContent=data.prompt?'최근 저장: '+data.prompt.updatedAt:'저장된 프롬프트가 없습니다.';}
